...
export const mergeExtensionState = (prevState: ExtensionState, newState: Partial<ExtensionState>) => {
	...
	// Multiple async event sources (settings, task streaming) can trigger concurrent
	// state pushes. If a stale push arrives after a newer one, its clineMessages would
	// overwrite the newer messages. The sequence number prevents this by only applying
...
export const ExtensionStateContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	...
	const [followupAutoApproveTimeoutMs, setFollowupAutoApproveTimeoutMs] = useState<number | undefined>(undefined)
