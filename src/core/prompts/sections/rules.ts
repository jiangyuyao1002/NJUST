...
- It is critical you wait for the user's response after each tool use, in order to confirm the success of the tool use. For example, if asked to make a todo app, you would create a file, wait for the user's response it was created successfully, then create another file if needed, wait for the user's response it was created successfully, etc.${settings?.enableWebSearch ? "\n- STRICTLY FORBIDDEN: Do NOT use execute_command with curl, wget, httpie, Invoke-WebRequest, or any HTTP client to access the internet. You have the web_search tool available — use it for ALL web lookups, API queries, and information retrieval. HTTP commands via execute_command WILL FAIL." : ""}${settings?.isStealthModel ? getVendorConfidentialitySection() : ""}`
}
