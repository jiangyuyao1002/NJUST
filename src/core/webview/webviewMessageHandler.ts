...
export const webviewMessageHandler = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	switch (message.type) {
		...
		// Simplified: no auth token needed
		try {
			const rooOptions = {
		...
		case "openKeyboardShortcuts": {
			// Open VSCode keyboard shortcuts settings and optionally filter to show the NJUST_AI_CJ commands
		...
		case "planAction": {
			...
			const planEngine = provider.planEngine
			const planId = planMsg.planId as string

			switch (planMsg.action) {
				case "approve":
					await planEngine.approvePlan(planId)
					await provider.postMessageToWebview({
						type: "planUpdate" as any,
						plan: planEngine.getPlan(planId),
					})
					break
				case "execute":
					planEngine
						...
						.catch((err) => {
							provider.log(`[PlanEngine] Execution error: ${err}`)
						})
					break
				case "pause":
					planEngine.pausePlan()
					break
				case "cancel":
					...
					break
				case "updateStep":
					planEngine.updateStep(planId, planMsg.stepId, {
						description: planMsg.description,
					})
					await provider.postMessageToWebview({
						type: "planUpdate" as any,
						plan: planEngine.getPlan(planId),
